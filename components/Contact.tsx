...
export default function Contact() {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.name.trim() || !formData.email.trim() || !formData.message.trim()) {
      setStatus("error");
      setStatusMessage(t.contact.form.emptyError);
      return;
    }
    ...
    setTimeout(() => {
      ...
      setStatus("success");
      setStatusMessage(t.contact.form.successMsg);
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-900/40 border border-slate-800/60 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[9px] font-bold tracking-wider text-slate-500 block uppercase font-mono">{t.contact.emailLabel}</span>
              ...
              <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-900/40 border border-slate-800/60 shadow-sm">
                ...
                <div>
                  <span className="text-[9px] font-bold tracking-wider text-slate-500 block uppercase font-mono">{t.contact.locationLabel}</span>
                  <span className="text-xs sm:text-sm font-bold text-slate-200">{t.contact.locationVal}</span>
                </div>
              </div>
            </div>
